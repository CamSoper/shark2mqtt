...
CELL_LABELS = [
    "Free", "Unknown", "Low-conf Free", "Navigable",
    "Partial (90%)", "Partial (92%)", "Wall", "Virtual Wall", "Other"
]
...
def render_floor_map(parsed, output_path=None, dpi=150, show_zones=True, show_boundaries=True):
    ...
    if parsed["pose"]:
        ...
        ax.text(
            px + 0.15, py - 0.3, "Robot",
            fontsize=8, color="#1565C0", fontweight="bold", zorder=6,
        )
        ...
        for i, zone in enumerate(parsed["zones"]):
            ...
            legend_patches.append(
                mpatches.Patch(
                    ...
                    label=f"Zone: {zone.get('zone_name', '')}"
                )
            )
    if show_boundaries and parsed["boundaries"]:
        legend_patches.append(
            mpatches.Patch(facecolor="none", edgecolor="#D32F2F", label="Obstacle boundary")
        )
    if parsed["pose"]:
        legend_patches.append(
            mpatches.Patch(color="#1565C0", label="Robot pose")
        )
